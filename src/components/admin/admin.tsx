...
export default function Admin() {
  ...
  const getFromAPI = () => {
    axios
      .get<IBooking[]>(baseURL + "booking/restaurant/" + ourRestaurantId)
      .then((response) => {
      ...
      .then((customerIdlist) => {
        ...
        for (let i = 0; i < customerIdlist.length; i++) {
          axios
            .get<ICostumerData[]>(baseURL + "customer/" + customerIdlist[i])
            .then((response) => {
  ...
  const sendUpdateBooking = () => {
    ...
    axios
      .put(baseURL + "booking/update/" + bookingIDForUpdate, {
        id: updateBooking.id,
        restaurantId: ourRestaurantId,
  ...
  const deletedBooking = (deletedBookingID: string, index: number) => {
    ...
    axios
      .delete(baseURL + "booking/delete/" + deletedBookingID)
      .then((response) => console.log(response))
      .then(getFromAPI)
      .catch((error) => {
        console.log("Det blev något fel!" + " Statuskod" + error);
    ...
    axios
      ...
      .catch((error) => {
        ...
        SetShowFreeTime(
          <div>
            <p>
              Blev ett fel med bokningen! Kolla att ni skrev in rätt info annars
              testa igen. Statuskod: {error}
            </p>
          </div>
        );
      });
    ...
    SetBookingUpdateInfo(
      <div className="change-booking">
        <p>
          ...
          Datum: {bookingDate}
          <br />
          Antal gäster: {bookingNumberOfGuest}
          <br />
          Skriv önskade ändring!
        </p>
      </div>
    );
  ...
  const confirmUpdatedBookingInfo = (
    chooseDate: string,
    choosenTime: string,
    ...
    for (let i = 0; i < bookingFromAPI.length; i++) {
      if (chooseDate === bookingFromAPI[i].date) {
        if (
          ...
          SetUpdateBookingText(
            "Tyvärr fullbokad men kanske kan kolla på en annat datum?"
          );
          SetUpdateBookingButton(true);
        } else {
          SetUpdateBookingText(
            "Kan ändra till det här datumet men en annan tid!!"
          );
          SetUpdateBookingButton(false);
        }
      }
  ...
  let bookinginfo = bookingFromAPI.map((bookings: IBooking, id: number) => {
    return (
      <div key={id} className="booking-info">
        <p>
  ...
  let personalBookingInfo = costumersFromAPI.map(
    (costumer: ICostumerData, id: number) => {
      return (
        <div key={id} className="customer-info">
          <p>
            Förnamn: {costumer.name}
            <br />
  ...
  const chooseWeek = (
    ...
    if (inputDateFromUser === "" || inputGuestFromUser === 0) {
      return SetShowFreeTime(
        <div>
          <p>
            Du valde inget datum eller antal gäster! gör om och försök igen!
          </p>
        </div>
      );
    } else {
  ...
  const bookingInfoHTML = (
    ...
    if (bookingFromAPI.length === 0) {
      ...
      return SetShowFreeTime(
        <div>
          <p>
            Det finns ett ledigt bord att boka gällande datumet: {chooseDate}{" "}
            och tiden: {choosenTime}
          </p>
          <SmallButton
            onClick={() => {
              newBookingHTML();
          ...
          for (let i = 0; i < bookingFromAPI.length; i++) {
            if (
              ...
              SetShowFreeTime(
                <div>
                  <p>
                    Det finns inget bord att boka gällande datumet: {chooseDate}{" "}
                    och tiden: {choosenTime}. <br />
              ...
              SetShowFreeTime(
                <div>
                  <p>
                    Det finns bord att boka gällande datumet: {chooseDate} och
                    tiden: {choosenTime}
                  </p>
                  <SmallButton
                    onClick={() => {
  ...
  return (
    <div className="App-header">
      <div className="admin-content">
        ...
        <Button
          onClick={() => {
            SetTooglePersonalInfo(!tooglePersonalInfo);
          }}
          disabled={bookingFromAPI.length == 0}>
          Visa personlig info
        </Button>

        <div hidden={!tooglePersonalInfo}>{bookinginfo}</div>

        <div hidden={tooglePersonalInfo}>{personalBookingInfo}</div>
